...
QVariant AlbumModel::data(const QModelIndex& index, int role) const
{
    ...
    const Album& album = *mAlbums->at(static_cast<unsigned int>(index.row()));
...
bool AlbumModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    Album& album = *mAlbums->at(static_cast<unsigned int>(index.row()));
...
bool AlbumModel::removeRows(int row, int count, const QModelIndex& parent)
{
    ...
    int countLeft = count;
    while (countLeft--) {
        const Album& album = *mAlbums->at(static_cast<unsigned int>(row + countLeft));
